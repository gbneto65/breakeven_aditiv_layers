...
def run_simul():
    ...
    fig.suptitle("Layer break-even  for  " + str(inputed_scr_farm_ident) + str(" - ") + str(todaysdate) + "\n" +
                 "Each hen should produce, at least, +" + str(
        round(st.median(addit_eggs), 1)) + " eggs in 700 days to be profitable\n", fontsize=10)
    # screen.state('zoomed')
    ...
    screen2.iconbitmap('layer_icon2.ico')
    ...
    #text.place(x = 750,
    #           y= 780)
    screen.maxsize(screen_x, screen_y)
    screen2.protocol("WM_DELETE_WINDOW", screen2.quit())
    screen2.mainloop()
...
screen.iconbitmap('layer_icon2.ico')
